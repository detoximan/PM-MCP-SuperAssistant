...
(function () {
  ...
  // ============================================
  // QWEN-ONLY: Network capture of tool calls
  // ============================================
  // On chat.qwen.ai the tool-call text is streamed by the server, but the
  // on-page Monaco editor virtualizes (hides) most lines, so reading the DOM is
  // unreliable -- the call only renders "every other time". We hook fetch here
  // (main world) to read the FULL assistant message straight from the network
  // stream and stash the PMTOOL block on <html data-qwen-net-toolcall> for the
  // content-script parser to use. Gated to qwen.ai ONLY, so z.ai and every other
  // site keep their current behavior.
  function isQwenHost() {
    try {
      return /(^|\.)qwen\.ai$/i.test(location.hostname);
    } catch (e) {
      return false;
    }
  }

  // Extract the PMTOOL.../PMTOOL block (with its markers) from anywhere in full.
  function qwenPmtoolBlock(full) {
    if (!full || full.indexOf('PMTOOL') === -1) return '';
    const lines = full.split(/\r?\n/);
    const start = lines.findIndex(l => l.trim() === 'PMTOOL');
    if (start === -1) return '';
    let end = -1;
    for (let i = start + 1; i < lines.length; i++) {
      if (lines[i].trim() === '/PMTOOL') {
        end = i;
        break;
      }
    }
  ...
  function publishQwenToolText(full) {
    try {
      const block = qwenPmtoolBlock(full);
      if (block) {
        document.documentElement.setAttribute('data-qwen-net-toolcall', block);
      }
    } catch (e) {
  ...
  if (isQwenHost() && !window.__qwenNetHooked) {
    window.__qwenNetHooked = true;
    const origFetch = window.fetch;
    window.fetch = async function (...args) {
      const url = (args[0] && args[0].url) || args[0] || '';
      const res = await origFetch.apply(this, args);
      try {
        if (typeof url === 'string' && url.indexOf('/chat/completions') !== -1 && res.body) {
          // New request: drop any block left over from the previous call so a
          // fresh card never picks up a stale command.
          document.documentElement.removeAttribute('data-qwen-net-toolcall');
          ...
          (async () => {
            ...
            for (;;) {
              ...
              while ((idx = buf.indexOf('\n\n')) !== -1) {
                ...
                buf = buf.slice(idx + 2);
                const dataLine = evt.replace(/^data:\s*/, '').trim();
    ...
    console.debug('[qwen-net] fetch hook installed');
  }
