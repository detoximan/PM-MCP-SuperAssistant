...
export class StreamableHttpPlugin implements ITransportPlugin {
  ...
  async getPrimitives(client: Client): Promise<any[]> {
    ...
    try {
      // NOTE: We intentionally do NOT gate these calls on getServerCapabilities().
      // The MCP SuperAssistant proxy aggregates multiple servers and often does
      // not advertise capabilities.tools/resources/prompts in its initialize
      // response, which previously caused listTools() to be skipped entirely
      // ("0 of 0 tools"). We always attempt all three and tolerate per-call errors.
      const capabilities = client.getServerCapabilities();
      logger.debug('[StreamableHttpPlugin] Server capabilities:', capabilities);
      ...
      promises.push(
        client.listTools().then(({ tools }) => {
          tools.forEach(item => {
            // Strip outputSchema: on Chrome MV3 the extension's schema-compile
            // path (Ajv new Function / unsafe-eval) is CSP-blocked, and an
            // outputSchema in a tool definition makes the UI drop ALL tools
            // ("0 of 0"). See MCP-SuperAssistant issues #191 / #171.
            const { outputSchema, ...toolWithoutOutputSchema } = (item ?? {}) as Record<string, unknown>;
            primitives.push({ type: 'tool', value: toolWithoutOutputSchema });
          });
        }).catch(error => {
