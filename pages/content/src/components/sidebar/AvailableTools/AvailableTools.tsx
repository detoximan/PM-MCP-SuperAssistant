  ...
  const { groupedTools, ungroupedTools } = useMemo(() => {
    ...
    filtered.forEach(tool => {
      ...
      if (dotIndex > 0) {
        ...
        // [TOOL_SUBGROUPS_v1] Split the big pm-filesystem server into functional sub-groups
        let groupName = serverName;
        if (serverName === 'pm-filesystem') {
          if (toolName.startsWith('notion_')) {
            ...
            toolName.startsWith('read_url') ||
            toolName === 'image_search' ||
            toolName === 'search_image_by_text'
          ) {
            groupName = 'Веб и поиск';
          } else {
            groupName = 'Файлы и Python';
          }
        }
