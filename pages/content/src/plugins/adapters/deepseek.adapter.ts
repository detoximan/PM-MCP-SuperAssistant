...
export class DeepSeekAdapter extends BaseAdapterPlugin {
  ...
  private readonly selectors = {
    ...
    CHAT_INPUT: 'textarea[placeholder*="Message DeepSeek"], textarea[placeholder*="Сообщение"], textarea[placeholder*="Ask"], textarea[spellcheck="false"], textarea[data-gramm="false"], textarea.chat-input, div[contenteditable="true"], textarea',
        ...
        if (button) {
          ...
          if (!isDisabled && isVisible) {
            try {
              ...
              this.context.logger.debug('DeepSeek submitted via submit button click');
              resolve(true);
              return true;
            } catch (error) {
  ...
  /**
   * Найти поле ввода DeepSeek, перебирая все известные селекторы.
   */
  ...
  private findSubmitButton(): HTMLElement | null {
    // Стратегия 1: по известным селекторам (aria-label на разных языках, data-testid).
