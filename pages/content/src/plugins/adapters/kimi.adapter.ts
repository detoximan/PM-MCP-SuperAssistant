...
export class KimiAdapter extends BaseAdapterPlugin {
  ...
  private readonly selectors = {
    ...
    CHAT_INPUT: '.chat-input-editor[contenteditable="true"], div[contenteditable="true"][data-lexical-editor="true"], .chat-input-editor, .chat-input, textarea[placeholder*="Ask"], textarea[placeholder*="Спроси"], textarea[placeholder*="Сообщение"], textarea[placeholder*="Введите"], div[contenteditable="true"], textarea',
  ...
  async submitForm(options?: { formElement?: HTMLFormElement }): Promise<boolean> {
    ...
    // Ищем кнопку отправки и, как только она станет активной, кликаем.
    // Если кнопку найти не удаётся вовремя — фолбэк на Enter.
    return new Promise<boolean>((resolve) => {
        ...
        if (button) {
          ...
          if (!isDisabled && isVisible) {
            try {
              ...
              this.context.logger.debug('Kimi submitted via submit button click');
              resolve(true);
              return true;
            } catch (error) {
      ...
      const intervalId = setInterval(() => {
        ...
        if (elapsed >= maxWaitTime) {
          ...
          this.tryEnterKeySubmission().then(resolve).catch(() => resolve(false));
        }
      }, checkInterval);
    });
  }
  ...
  /**
   * Найти поле ввода Kimi, перебирая все известные селекторы.
   */
  ...
  /**
   * Найти кнопку отправки несколькими способами (русский/английский
   * интерфейс, кнопка-стрелка без подписи).
   */
  private findSubmitButton(): HTMLElement | null {
    // Стратегия 1: по известным селекторам (классы Kimi + aria-label на разных языках, data-testid).
    ...
    for (const selector of directSelectors) {
      ...
      if (el) {
        this.context.logger.debug(`Found submit button via selector: ${selector}`);
        return el;
      }
    }

    // Стратегия 2: эвристика — кнопка-стрелка рядом с полем ввода.
    // Поднимаемся от поля ввода к его контейнеру и берём самую правую
    // кликабельную кнопку со svg (обычно это стрелка отправки).
    ...
    if (input) {
      ...
      const withSvg = candidates.filter((el) => {
        if (el.closest('#mcp-popover-container')) return false; // не наша кнопка MCP
        const rect = el.getBoundingClientRect();
        return !!el.querySelector('svg') && rect.width > 0 && rect.height > 0;
      });

      if (withSvg.length > 0) {
        // Стрелка отправки обычно крайняя справа.
        withSvg.sort(
  ...
  private async tryEnterKeySubmission(): Promise<boolean> {
    try {
      // Find the chat input element (мультиязычный поиск, не только первый селектор)
      ...
      // Полная серия событий Enter (без Shift): keydown, keypress, keyup.
      ...
      // Доп. фолбэк: отправка формы, если поле внутри form.
