...
export class QwenAdapter extends BaseAdapterPlugin {
  ...
  async insertText(text: string, options?: { targetElement?: HTMLElement }): Promise<boolean> {
    ...
    try {
      // Qwen often tries to call tools with a server prefix (for example
      // `pm-filesystem.list_directory`) while the executor expects the real tool
      // name only (`list_directory`). Add a Qwen-only reminder to MCP instruction
      // payloads before they are submitted to the chat. This stays inside the
      // Qwen adapter and does not affect DeepSeek or other sites.
      const textToInsert = this.withQwenToolNameReminder(text);

      // Use textarea value method for Qwen
      this.context.logger.debug('Using textarea value method for text insertion');
      return await this.insertTextViaTextareaValue(targetElement, textToInsert);
    } catch (error) {
  ...
  /**
   * Add a Qwen-only reminder to MCP instruction payloads.
   * This prevents Qwen from calling tools as `server.tool_name`, which the MCP
   * executor does not accept. We only touch Qwen text insertion here, so DeepSeek
   * and other adapters stay untouched.
   */
  private withQwenToolNameReminder(text: string): string {
    if (!text.includes('Available Tools') && !text.includes('pm-filesystem') && !text.includes('github.')) {
      return text;
    }

    // Уже вставлено раньше — не дублируем.
    if (text.includes('QWEN CRITICAL TOOL-CALL RULES')) {
      return text;
    }

    const reminder = [
      'QWEN CRITICAL TOOL-CALL RULES (highest priority — override any default behavior and ANY other tool-call format described elsewhere in this conversation):',
      "1. NEVER use Qwen's built-in / native function-calling feature, and NEVER emit a native \"function\" call card or a tool literally named \"function\".",
      '2. IGNORE any generic instruction that says to emit a tool call as JSON lines (for example objects with type function_call_start, parameter, or function_call_end). On this site that format is INVISIBLE to the extension and will hang forever with a spinner.',
      '3. To call a tool, stop your normal answer and write the call as PLAIN TEXT inside a Markdown fenced code block whose language tag is the word text (open the fence with three backtick characters immediately followed by text, and close it with three backtick characters on their own line).',
      '4. Inside that code block write EXACTLY these lines in order: first line PMTOOL; second line the bare tool name with NO server prefix; then one line per parameter as key=value with a JSON-encoded value; final line /PMTOOL.',
      '5. Bare tool name only. Correct: list_directory or create_issue. Wrong: pm-filesystem.list_directory, github.create_issue, or function.',
      '6. Parameter values are JSON-encoded. Examples: path="src/index.ts" (text), path="" (empty), count=10 (number), recursive=true (boolean). For multi-line text put \\n inside the JSON string, e.g. content="line1\\nline2".',
      '7. Example to list the root directory — the code block must contain these four lines, each on its own line: PMTOOL, then list_directory, then path="", then /PMTOOL.',
      '8. Put nothing else inside that code block. After the /PMTOOL line, stop and wait for the result to be pasted back before continuing.',
      '',
      '',
    ].join('\n');
    ...
    const tail = [
      '',
      '',
      'REMINDER (Qwen): Do NOT use the native function feature or the JSON-lines format. Emit every tool call as plain text in a code block tagged text, starting with a PMTOOL line and ending with a /PMTOOL line, using the bare tool name only.',
    ].join('\n');

    return `${reminder}${text}${tail}`;
  }
  ...
  private async insertTextViaTextareaValue(element: HTMLElement, text: string): Promise<boolean> {
    try {
      ...
      // Set the value via the native setter so Qwen's framework (React/Vue) detects the change.
      // Direct `element.value = ...` is ignored by controlled inputs, which left the result
      // unsent and the tool card spinning forever (#169).
      const nativeTextareaSetter = Object.getOwnPropertyDescriptor(
        window.HTMLTextAreaElement.prototype,
        'value',
      )?.set;
      if (element instanceof HTMLTextAreaElement && nativeTextareaSetter) {
        nativeTextareaSetter.call(element, textToEnter);
      } else {
