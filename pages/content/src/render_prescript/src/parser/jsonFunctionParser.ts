 ...
 */
/**
 * QWEN-ONLY FIX (smart quotes): Qwen Chat (chat.qwen.ai) renders tool calls in a
 * code widget and replaces straight quotes (") with smart/curly quotes
 * (“ ” „ « » ‘ ’). That breaks JSON.parse and the detection regexes, so the
 * call is never executed. These helpers repair the text and are gated to qwen.ai
 * ONLY — DeepSeek, Kimi, Google AI Studio and every other site keep their exact
 * current behavior.
 */
const isQwenSite = (): boolean => {
  try {
    return typeof window !== 'undefined' && /(^|\.)qwen\.ai$/i.test(window.location.hostname);
  } catch {
    return false;
  }
};

export const normalizeSmartQuotes = (text: string): string => {
...
const parseJSONLine = (line: string): JSONFunctionLine | null => {
  try {
    ...
    try {
      ...
      // QWEN-ONLY repair: smart/curly quotes break JSON.parse. Retry once with
      // normalized quotes. Other sites fall through to the old behavior.
      if (isQwenSite()) {
        parsed = JSON.parse(normalizeSmartQuotes(cleaned));
      } else {
        throw parseError;
      }
    }
...
/**
 * QWEN-ONLY: Read the plain-text PMTOOL tool-call block and convert it into the
 * standard JSONL the rest of the pipeline understands.
 *
 * On chat.qwen.ai the model is instructed to emit tool calls as a plain ```text
 * code block (Qwen does NOT hijack that into a native widget), in this shape:
 *   PMTOOL
 *   <tool_name>
 *   <param>=<json_value>
 *   /PMTOOL
 * We must read `innerText` (not `textContent`): on Qwen code blocks the
 * line-number gutter gets glued to the front and the newlines are dropped in
 * `textContent`, while `innerText` keeps the line breaks we need. Any leading
 * server prefix (e.g. `pm-filesystem.`) is stripped so the executor receives the
 * bare tool name. `function_call_end` is emitted only once the closing `/PMTOOL`
 * marker is present, so the card stays in the loading state while the call is
 * still streaming. Returns '' on other sites or when no PMTOOL block is found,
 * so all non-Qwen behavior is unchanged.
 */
export const getQwenFullCallContent = (block: HTMLElement | null): string => {
  try {
    if (!isQwenSite() || !block) return '';
    // innerText preserves line breaks; textContent does not on Qwen code blocks.
    let raw = block.innerText || block.textContent || '';
    // Qwen's on-page Monaco editor virtualizes (hides) most lines, so the DOM
    // text is frequently incomplete and the call only renders "every other time".
    // The main-world fetch hook in codemirror-accessor.js reads the FULL assistant
    // message from the network stream and publishes the PMTOOL block on
    // <html data-qwen-net-toolcall>. When the DOM copy is not yet complete (no
    // closing /PMTOOL marker), use the network copy instead.
    try {
      const net = document.documentElement.getAttribute('data-qwen-net-toolcall');
      if (net && net.indexOf('PMTOOL') !== -1 && raw.indexOf('/PMTOOL') === -1) {
        raw = net;
      }
    } catch {
      /* fall back to DOM text */
    }
    if (!raw || raw.indexOf('PMTOOL') === -1) return '';

    const text = normalizeSmartQuotes(raw);
    const lines = text.split(/\r?\n|\u2028|\u2029/).map(l => l.trim());

    const startIdx = lines.findIndex(l => l === 'PMTOOL');
    if (startIdx === -1) return '';
    const endIdx = lines.findIndex((l, i) => i > startIdx && l === '/PMTOOL');

    // Body = everything between the markers (or to the end while streaming).
    // Drop empty lines and stray numeric gutter lines.
    const bodyEnd = endIdx === -1 ? lines.length : endIdx;
    const body = lines
      .slice(startIdx + 1, bodyEnd)
      .filter(l => l.length > 0 && !/^\d+$/.test(l));
    if (body.length === 0) return '';

    // First line is the tool name; strip any leading server prefix.
    const name = body[0].replace(/^[\w-]+\./, '');
    if (!name) return '';

    const out: string[] = [];
    out.push(JSON.stringify({ type: 'function_call_start', name, call_id: 1 }));
    ...
    for (let i = 1; i < body.length; i++) {
      ...
      try {
        ...
        value = rawVal; // bare (non-JSON) value, including '' for empty
      }
      out.push(JSON.stringify({ type: 'parameter', key, value }));
    }

    // Only mark the call complete when the closing marker has arrived.
    if (endIdx !== -1) {
      out.push(JSON.stringify({ type: 'function_call_end', call_id: 1 }));
    }
...
export const containsJSONFunctionCalls = (block: HTMLElement): FunctionInfo => {
  ...
  // QWEN-ONLY Priority 0: a single tool call is split across the code <pre>
  // (only function_call_start) and Qwen's native widget (description/parameter/
  // function_call_end). Reading just this <pre> never sees function_call_end, so
  // the call never completes (eternal spinner). Rebuild the full call from the
  // whole message container. No-op (returns '') on every other site.
  if (!content) {
    const qwenFull = getQwenFullCallContent(block);
    if (qwenFull) {
      content = qwenFull;
    }
  }
  ...
  // QWEN-ONLY: repair smart/curly quotes so the quick-check regexes below and the
  // per-line JSON.parse can recognize the call. No-op for other sites.
  if (isQwenSite() && content) {
