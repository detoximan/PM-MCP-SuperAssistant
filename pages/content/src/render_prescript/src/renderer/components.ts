...
export const addExecuteButton = (blockDiv: HTMLDivElement, rawContent: string): void => {
  ...
  executeButton.onclick = async () => {
    // [MCP_DEDUP_v1] Global per-call execution lock. The same tool call
    // (functionName + call_id) can be triggered from several places at once:
    // the built-in auto-exec, site-specific watchers (e.g. Xiaomi), and DOM
    // re-renders. Without this guard the SAME call ran 3-4 times. We allow the
    // first run and ignore any repeat of the same call within a short window.
    // On single-trigger sites this guard simply never fires, so their behavior
    // is unchanged. A deliberate manual re-run after the window still works.
    const __mcpExecKey = `${functionName}::${callId}`;
    const __mcpExecNow = Date.now();
    const __mcpExecLocks = ((window as any).__mcpCallExecLock ||= {} as Record<string, number>);
    const __mcpExecLast = __mcpExecLocks[__mcpExecKey];
    if (typeof __mcpExecLast === 'number' && __mcpExecNow - __mcpExecLast < 6000) {
      logger.debug(`[MCP_DEDUP_v1] Ignoring duplicate execution of ${__mcpExecKey} (${__mcpExecNow - __mcpExecLast}ms since last run)`);
      return;
    }
    __mcpExecLocks[__mcpExecKey] = __mcpExecNow;
    ...
    try {
      ...
      try {
        ...
        resetButtonState();
        // [MCP_DEDUP_v1] Execution failed: release the lock so a manual retry works immediately.
        delete __mcpExecLocks[__mcpExecKey];
