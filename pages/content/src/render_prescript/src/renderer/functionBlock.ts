...
import { extractJSONFunctionInfo, extractJSONParameters, getQwenFullCallContent } from '../parser/jsonFunctionParser';
...
export const renderFunctionCall = (block: HTMLPreElement, isProcessingRef: { current: boolean }): boolean => {
  ...
  let rawContent = block.textContent?.trim() || '';
  // QWEN-ONLY: the code <pre> holds only function_call_start; rebuild the full
  // call (description/parameter/function_call_end) from the whole message
  // container so name, params and completion are correct. '' on other sites.
  const qwenFullContent = getQwenFullCallContent(block);
  if (qwenFullContent) {
    rawContent = qwenFullContent;
  }
